...
metrics=struct('LF_ms2',NaN,'HF_ms2',NaN,'LF_HF_ratio',NaN,...
    'FrequencyDuration_sec',NaN,'FrequencyEligible',false);
if numel(nn)<2 || numel(t)~=numel(nn), return; end
duration=t(end)-t(1); metrics.FrequencyDuration_sec=duration;
if numel(nn)<16 || duration<cfg.minFrequencyDurationSec, return; end
tu=(t(1):1/cfg.frequencyResampleHz:t(end))';
yu=detrend(interp1(t,nn,tu,'pchip'));
[pxx,f]=pwelch(yu,[],[],max(256,2^nextpow2(numel(yu))),...
    cfg.frequencyResampleHz);
...
metrics.FrequencyEligible=true;
end
