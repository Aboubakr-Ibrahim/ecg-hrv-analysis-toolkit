...
[nnMs,nnTimeSec,cleaning]=clean_nn_intervals(rPeaks,fs,cfg);
metrics=compute_time_domain_hrv(nnMs,cfg);
frequency=compute_frequency_domain_hrv(nnMs,nnTimeSec,cfg);
poincare=compute_poincare_hrv(nnMs);
...
result=struct('recordId',recordId,'fs',fs,'signal',signal,'rPeaks',rPeaks,...
    'nnMs',nnMs,'nnTimeSec',nnTimeSec,'metrics',metrics,...
